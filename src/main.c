...
file_type_t get_file_type(const char *filename) {
    ...
    // Check if it's a directory (this would need to be determined from the dir listing)
    // For now, we'll assume it's a regular file
...
void print_usage(const char *program_name) {
    ...
    printf("  baud_rate:   Baud rate (default: 9600)\n");
...
int main(int argc, char *argv[]) {
    ...
    if (argc >= 3) {
        ...
        switch (baud_rate) {
            ...
            default:
                printf("Invalid baud rate. Using 9600.\n");
                baud_rate = B9600;
                break;
        }
    }
    ...
    if (cisco_init_flash(&app.serial, 6) < 0) {
        ui_cleanup();
