 ...
 */
int mock_serial_read_until_func(serial_conn_t *conn, char *buffer, int max_len,
...
int mock_serial_write_func(serial_conn_t *conn, const char *data) {
  ...
  if (data) {
    strncpy(mock_serial_write.last_data, data,
            sizeof(mock_serial_write.last_data) - 1);
...
int serial_read_until(serial_conn_t *conn, char *buffer, int max_len,
                      const char *delimiter) {
 ...
 */
TEST_SETUP(serial) { MOCK_INIT_ALL(); }
...
static int run_single_test(const char *test_name, test_func_t test_func) {
  ...
  // Suppress stdout during test execution to avoid printf output from functions
  // under test
  suppress_stdout();
...
int is_binary_file(const char *filename) {
  ...
  if (ext) {
    if (strcmp(ext, ".bin") == 0 || strcmp(ext, ".exe") == 0 ||
        strcmp(ext, ".dll") == 0 || strcmp(ext, ".so") == 0 ||
        strcmp(ext, ".dylib") == 0 || strcmp(ext, ".o") == 0 ||
        strcmp(ext, ".a") == 0) {
