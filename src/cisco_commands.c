 ...
 *
 * This function sends a command string to the Cisco device via serial
 * connection, automatically appending a newline character. After sending the
 * command, it waits for the device to respond and return to the command prompt.
 *
 * @param conn Pointer to the serial connection structure
 * @param command The command string to send (without newline)
 * @param timeout Maximum time to wait for response in seconds
 *
 * @return 0 on success, -1 on error (write failure or timeout waiting for
 * prompt)
 *
 ...
 *
 * This function waits for the "flash_init" prompt to appear from the Cisco
 * device, then sends the "flash_init" command to initialize the flash
 * filesystem. This is typically required before performing file operations on
 ...
 * of the specified directory. It parses the response to extract file
 * information including names, sizes, permissions, and types. The results are
 * stored in a linked list of file_entry_t structures.
 *
 * @param conn Pointer to the serial connection structure
 ...
 *
 * @note The caller is responsible for freeing the allocated file_entry_t
 * structures when they are no longer needed. The function handles special cases
 * for "flash:/" paths and automatically determines file types based on
 * permissions and filename extensions.
 *
 * @warning This function allocates memory for each file entry found. Memory
 * leaks will occur if the returned list is not properly freed.
 */
int cisco_get_directory_listing(serial_conn_t *conn, const char *path,
                                file_entry_t **files, int timeout) {
  ...
  *files = NULL;
  ...
  while (line != NULL) {
    ...
    if (strlen(line) == 0 || strstr(line, "Directory of") != NULL ||
    ...
    // Parse file entry (format: "2  -rwx  1429      Jan 01 2020 00:00:00 +00:00
    // filename")
    int file_num, size;
    char permissions[16];
    ...
    if (last_space != NULL && last_space != line) {
      ...
      // Simple parsing approach - look for the pattern: number, permissions,
      // size, filename Format: "    2  -rwx  1429      <date> filename" Use a
      // more flexible pattern that skips leading whitespace
      if (sscanf(line, " %d %s %d", &file_num, permissions, &size) == 3 &&
        ...
        if (strcmp(path, "flash:/") == 0) {
          ...
          if (filename_len >
              MAX_PATH_LEN -
                  8) { // 8 = strlen("flash:/") + 1 for null terminator
            ...
            // Use strcpy and strcat instead of snprintf to avoid truncation
            // warnings
          ...
          size_t filename_len = strlen(file->name);
          if (path_len + filename_len + 2 >
              MAX_PATH_LEN) { // +2 for "/" and null terminator
            // Truncate to fit
            strncpy(file->full_path, path, MAX_PATH_LEN - 1);
            file->full_path[MAX_PATH_LEN - 1] = '\0';
            strncat(file->full_path, "/",
                    MAX_PATH_LEN - strlen(file->full_path) - 1);
            strncat(file->full_path, file->name,
                    MAX_PATH_LEN - strlen(file->full_path) - 1);
            file->full_path[MAX_PATH_LEN - 1] = '\0';
          } else {
            // Use strcpy and strcat instead of snprintf to avoid truncation
 ...
 *
 * This function sends a "delete" command to remove a file from the Cisco
 * device's flash memory. It automatically handles the confirmation prompt that
 * Cisco devices typically display before deleting files.
 *
 * @param conn Pointer to the serial connection structure
 * @param file_path The full path to the file to delete (e.g.,
 * "flash:/config.txt")
 * @param timeout Maximum time to wait for response in seconds
 *
 ...
 *       If the file doesn't exist or cannot be deleted, the function will
 * return -1.
 *
 ...
 *
 * This function sends an "rmdir" command to remove a directory from the Cisco
 * device's flash memory. It automatically handles the confirmation prompt that
 * Cisco devices typically display before removing directories.
 *
 * @param conn Pointer to the serial connection structure
 * @param dir_path The full path to the directory to delete (e.g.,
 * "flash:/backup")
 * @param timeout Maximum time to wait for response in seconds
 *
 ...
 *
 * @note The function automatically responds "y" to the confirmation prompt.
 *       The directory must be empty for the removal to succeed. If the
 * directory doesn't exist or cannot be removed, the function will return -1.
 *
